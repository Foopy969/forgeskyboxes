...
public class Rotation {
    public static final Rotation DEFAULT = new Rotation(true, new Vector3f(0F, 0F, 0F), new Vector3f(0F, 0F, 0F), new BlockPos(0, 0, 0), 0, 0, 0);
    public static final Rotation DECORATIONS = new Rotation(false, new Vector3f(0F, 0F, 0F), new Vector3f(0F, 0F, 0F), new BlockPos(0, 0, 0), 0, 0, 1);
    ...
    private static final Codec<BlockPos> VEC_3_I = Codec.INT.listOf().comapFlatMap((list) -> {
        if (list.size() < 3) {
            return DataResult.error("Incomplete number of elements in vector");
        }
        return DataResult.success(new BlockPos(list.get(0), list.get(1), list.get(2)));
    ...
    public static final Codec<Rotation> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            VEC_3_I.optionalFieldOf("timeShift", new BlockPos(0, 0, 0)).forGetter(Rotation::getTimeShift),
    ...
    private final BlockPos timeShift;
    private final float rotationSpeedX;
    private final float rotationSpeedY;
    private final float rotationSpeedZ;

    public Rotation(boolean skyboxRotation, Vector3f staticRot, Vector3f axisRot, BlockPos timeShift, float rotationSpeedX, float rotationSpeedY, float rotationSpeedZ) {
